...
Fixpoint predicate_to_prop `{Env K} (p : predic K ) (en : env K)
  (rho : stack K) (m : mem K) : option Prop :=
  match p with
  | PTrue => Some True
  | PFalse => Some False
  | PRelOp op t1 t2  => let v1 := term_eval_right t1 en rho m in
                        let v2 := term_eval_right t2 en rho m in
                        match v1,v2 with
                        |Some(val1), Some(val2) => Some (comp_valt op en val1 val2) 
                        |_,_ => None
                        end
  | POr p1 p2 =>  let prop1 := predicate_to_prop p1 en rho m in
                  let prop2 := predicate_to_prop p2 en rho m in
                  match prop1,prop2 with
                  | Some p1', Some p2' => Some (p1' \/ p2')
                  | _,_ => None
                  end
  | PAnd p1 p2 => let prop1 := predicate_to_prop p1 en rho m in
                  let prop2 := predicate_to_prop p2 en rho m in
                  match prop1,prop2 with
                  | Some p1', Some p2' => Some (p1' /\ p2')
                  | _,_ => None
                  end
  | PImpl p1 p2 => let prop1 := predicate_to_prop p1 en rho m in
                   let prop2 := predicate_to_prop p2 en rho m in
                   match prop1,prop2 with
                   | Some p1', Some p2' => Some (p1' -> p2')
                   | _,_ => None
                   end
  | PNot p1 => let prop1 := predicate_to_prop p1 en rho m in
               match prop1 with
               | Some p1' => Some (~p1')
               | _ => None
               end
  | PValid t1 => let v1 := term_eval_right t1 en rho m in
                 match v1 with
  ...
  (* Initialized *)
  | PInit t1 => let v1 := term_eval_right t1 en rho m in
                  match v1 with
                  |Some (val1) => match val1 with
                                  | ValC (VBase (VPtr p)) => match p with
                                                      | NULL _ => Some False
